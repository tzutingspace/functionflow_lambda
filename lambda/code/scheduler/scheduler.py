...
def lambda_handler(event, context):
    ...
    with connection.cursor() as cursor:
        # 找出本次符合條件的 workflow
        sql = 'SELECT id, name, trigger_type, next_execute_time FROM workflows \
            WHERE status = "active" \
    ...
    for work_flow in work_flows:
        ...
        with connection.cursor() as cursor:
            ...
            work_flow["execution_time"] = current_date_and_time.strftime("%Y-%m-%d %H:%M:%S")
            ...
            sql = "SELECT jobs.id, jobs.name, jobs.sequence, jobs.depends_job_id,\
                ...
                FROM `jobs` INNER JOIN `functions` ON jobs.function_id = `functions`.id \
                    WHERE workflow_id = %(id)s ORDER BY sequence"
            cursor.execute(sql, work_flow)
        ...
        with connection.cursor() as cursor:
            ...
            for job in job_details:
                ...
                job["start_time"] = None
                job["end_time"] = None
                job["result_output"] = None
                QUEUE_OBJ["ready_execute_job"].append(job["name"])
                if job["sequence"] == 1:
